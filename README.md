...
* Using **local wifi** Eliminates the need for third-party cloud services or external server infrastructure.
* **Local Web Interface:** Runs a lightweight embedded HTTP server directly on the Android device, accessible via any desktop browser on the same Wi-Fi network.
* **File Management:** Provides seamless access to device files, media, and internal storage without transmitting data externally.
...
* **Language:** Dart
* **Framework:** Flutter
* **Server:** Embedded HTTP server using flutter http
* **Architecture:** Dual app architecture for implementing web interface with android app
